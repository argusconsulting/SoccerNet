...
import {
  api_name_create_comments,
  api_name_get_comments,
  api_name_like_post,
  api_name_post,
  ...
  async ({post_id}) => {
    try {
      const response = await getApi(
        `${api_name_get_comments}?post_id=${post_id}&limit=50&start=0`,
      );
...
export const discussionSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      .addCase(postHandler.pending, (state, action) => {
    ...
    //like post
    builder
      .addCase(likePostHandler.pending, (state, action) => {
        state.isLoading = true;
      })
