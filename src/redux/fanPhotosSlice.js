...
import { api_name_fan_photos, api_name_post_fan_photos } from "../constants/api-constants";
...
export const getFanPhotos = createAsyncThunk('fanPhotos/photos', async () => {
  ...
  export const postFanPhotos = createAsyncThunk('postFanPhotos/photos', async ({caption, image , acknowledge}) => {
    try {
  ...
  export const fanPhotosSlice = createSlice({
    ...
    extraReducers: builder => {
      ...
      builder.addCase(postFanPhotos.pending, (state, action) => {
        state.isLoading = true;
      })
