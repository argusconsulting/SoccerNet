...
const RoomsListing = () => {
  ...
  const DATA = [
    ...
    },
    {
      ...
      shortDesc: 'Discuss the latest movies and reviews.Chat about the latest in tech.',
      userIcons: [
  ...
  const Item = ({ item }) => (
    <View style={tw`bg-[#303649] p-4 rounded-lg w-43.5 m-2`}>
      <Text style={tw`text-white text-[20px] font-401 leading-normal mb-1`}>{item.roomName}</Text>
      ...
      <View style={tw`flex-row items-center mb-2`}>
        {item.userIcons.map((user, index) => (
          ...
          >
            <Image source={{ uri: user.uri }} style={[tw`w-full h-full`, { resizeMode: 'contain' }]} />
          </View>
        ))}
