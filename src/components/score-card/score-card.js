...
const ScoreCard = React.memo(({match, width, screen, navigate}) => {
  ...
  return (
    <TouchableOpacity
      ...
      <View style={tw`flex-row justify-between mx-3 `}>
        <Image
          source={{uri: match?.league?.image_path}}
          style={tw`w-9 h-9 mt-2 `}
        />
        {screen && (
          <Text
            style={tw`text-[#a9a9a9] text-[20px] font-400 leading-normal self-center mt-1.5 `}>
            {moment(match?.starting_at).format('MMMM Do YYYY')}
          </Text>
        )}
        {screen === 'liveNow' ? (
          <Text
            style={tw`text-red-500 text-[20px] font-401 leading-normal mt-1.5 `}>
            Live
          </Text>
        ) : (
          <Text
            style={tw`text-[#fff] text-[20px] font-401 leading-normal mt-1.5 `}>
            {match?.length}"
          </Text>
        )}
      </View>
  ...
  <Text
    style={[
      tw`text-[#fff] text-[18px] font-400 leading-tight mt-1.5 w-29 self-center`,
      { textAlign: "center" }
    ]}
  >
    {homeTeam?.name.length > 9
  ...
  :   
  <Text
            style={[tw`text-[#fff] text-[18px] font-400 leading-tight mt-1.5 w-23 self-center` ,{textAlign:"center"}]}>
            {homeTeam?.name}
          </Text>
}
        </View>
        <Text
          style={tw`text-[#fff] text-[22px] font-401 leading-normal mt-1.5 ml-3`}>
          {homeScore}
        </Text>
        <Text
          style={tw`text-[#fff] text-[22px] font-401 leading-normal mt-1.5 `}>
          -
        </Text>
        <Text
          style={tw`text-[#fff] text-[22px] font-401 leading-normal mt-1.5 mr-3`}>
          {awayScore}
        </Text>
        <View style={tw`w-[40%] `}>
          <View
            ...
            <Image
              source={{uri: awayTeam?.image_path}}
              style={[
                tw`w-12 h-12 self-center mt-1.5`, // Image size and centering
                {resizeMode: 'contain', borderRadius: 999}, // Make the image circular
              ]}
            />
          </View>
